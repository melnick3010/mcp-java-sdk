 ...
 *
 * Test white-box: non apre connessioni HTTP; esercita la logica di connect(...)
 * su un singolo "incoming" e usa una sottoclasse che intercetta sendMessage(...).
 */
public class HttpClientSseClientTransportDispatchSmokeTest {
    ...
    static class SpyTransport extends HttpClientSseClientTransport {
        ...
        int getPostCount() { return postCount.get(); }
        ...
        /**
         * Metodo di comodo per il test: esercita la stessa logica branching di connect(...)
         * per un singolo "incoming" senza eseguire I/O.
         */
        Mono<Void> dispatchIncomingForTest(JSONRPCMessage incoming,
            ...
            if (incoming instanceof McpSchema.JSONRPCRequest) {
                ...
                // Response/Notification: non deve postare nulla; lascia completare localmente
                return out.then();
            }
        }
    }
    ...
    void incomingResponse_shouldNotPost() {
        ...
        McpSchema.JSONRPCResponse resp = new McpSchema.JSONRPCResponse(
            McpSchema.JSONRPC_VERSION,
            "client-0", /* id di una request del client */
    ...
    void incomingRequest_shouldPostOnce() {
        ...
        McpSchema.JSONRPCRequest req = new McpSchema.JSONRPCRequest(
            McpSchema.JSONRPC_VERSION,
            "ping",
            "server-1",
        ...
        // Handler del client: produce la response (echo dell'id) che il transport deve postare
        Function<Mono<JSONRPCMessage>, Mono<JSONRPCMessage>> handler = mono ->
            mono.map(in -> {
                McpSchema.JSONRPCRequest r = (McpSchema.JSONRPCRequest) in;
                return new McpSchema.JSONRPCResponse(
                    McpSchema.JSONRPC_VERSION,
                    r.id(), /* echo id */
                    java.util.Collections.singletonMap("pong", true),
                    null
                );
            });
        ...
        assertThat(t.getPostCount())
            .as("POST deve essere chiamata UNA volta per JSONRPCRequest")
            .isEqualTo(1);
    }
}
